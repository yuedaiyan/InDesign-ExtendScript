/*
  文件: ReplaceMissingImagesFromFolder.jsx

  用途:
  - 扫描当前文档中链接缺失的图片，并从指定文件夹中按文件名寻找替换文件。

  使用前:
  - 打开包含缺失链接的 InDesign 文档。
  - 准备包含替换图片的文件夹。
  ...
  注意:
  - 缺失链接按页面阅读顺序处理，文件夹图片按自然文件名排序。
  - 实际替换使用 frame.place()，绕开缺失链接逐个更新造成的卡顿。
  - 全部替换包装为一次撤销操作。
*/
(function () {
    try {
