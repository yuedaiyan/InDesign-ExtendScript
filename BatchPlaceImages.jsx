/*
  文件: BatchPlaceImages.jsx

  用途:
  - 批量将图片按文件名自然排序置入当前文档的空白图片框。
  - 可从当前选中空框、指定页码第一个空框或文档第一个空框开始。

  使用前:
  - 打开要处理的 InDesign 文档。
  - 如果希望从特定位置开始，可先单选一个空 Rectangle / Oval / Polygon 图片框。
  - 准备好图片文件夹，或准备好作为起点的单张图片。

  运行流程:
  1. 运行脚本后选择图片来源和置入起点。
  2. 选择填充/适合方式。
  3. 查看脚本给出的置入数量和起点信息，确认后执行。

  注意:
  - 只处理可用的空图片框；已有图片、文本框、锁定对象和隐藏/锁定图层会被跳过。
  - 置入动作包装为一次撤销操作。
*/
(function () {
    try {
