 ...
 *
 * 用途:
 *   将右侧图片组中各图片框的链接文件名(去除扩展名)
 *   填写到左侧文本框组中位置一一对应的文本框里。
 *
 * 使用流程:
 *   1. 在 InDesign 中跨页同时选中两个 Group:
 *      左侧文本框组(偶数页) + 右侧图片框组(奇数页)
 *   2. 运行此脚本
 *   3. 输入页码范围(如 2-20),脚本会对范围内每一对
 *      "偶数页文字组 + 奇数页图片组" 执行相同逻辑
 *
 * 规则:
 *   - 组内排序:从左到右,从上到下,行容差 3mm
 *   - 文件名去除扩展名后写入
 *   - 空图片框对应的文本框不写入(保持原状不变)
 *   - 非空图片框:文本框已有内容会被覆盖
 *   - 只处理可见且未锁定的元素
 */
