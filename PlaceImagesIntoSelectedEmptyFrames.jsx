/*
  文件: PlaceImagesIntoSelectedEmptyFrames.jsx

  用途:
  - 把图片文件夹中的图片按文件名顺序置入当前选中的空图片框。
  - 同时按阅读顺序为框架写入 name 和 label。

  使用前:
  - 打开要处理的 InDesign 文档。
  - 选中一个或多个空 Rectangle / Oval / Polygon 图片框。
  - 准备包含图片的文件夹。

  运行流程:
  1. 运行脚本。
  2. 输入基础标签，脚本会生成 基础标签_1、基础标签_2 等。
  3. 选择图片文件夹，并指定从第几张图片开始置入。
  4. 确认后执行命名、打标签和置图。

  注意:
  - 图片不足时，剩余框只命名和打标签，不置入图片。
  - 全部操作包装为一次撤销操作。
*/
(function () {
    try {
