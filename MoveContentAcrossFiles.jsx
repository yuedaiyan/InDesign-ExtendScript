/*
  文件: MoveContentAcrossFiles.jsx

  用途:
  - 从一个已打开 InDesign 文档的指定图层和页码范围，复制页面对象到另一个已打开文档的指定图层和页码范围。

  使用前:
  - 至少打开源文档和目标文档。
  - 确认源/目标文档名、源/目标图层名、起始页和结束页。
  - 默认会弹出设置窗口；也可以修改脚本顶部配置后直接运行。

  运行流程:
  1. 运行脚本。
  2. 在设置窗口中确认文档、图层和页码范围。
  3. 脚本按页复制对应图层上的对象，并输出统计报告。

  注意:
  - 适合源文件和目标文件页面尺寸、页面结构基本一致的情况。
  - 脚本使用 duplicate(targetPage)，不是依赖剪贴板。
*/
(function () {
    // =========================
    // 你需要修改的配置
